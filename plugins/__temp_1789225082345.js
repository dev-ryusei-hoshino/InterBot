...
const YTDLP_PATH = "C:\\Users\\rayyt\\AppData\\Local\\Python\\pythoncore-3.14-64\\Scripts\\yt-dlp.exe";
...
export default {
  ...
  async run(conn, m, { jid, args, usedPrefix, command }) {
    ...
    try {
      ...
      const v = videos[0];
      console.log("[YT PLAY] selected video:", JSON.stringify({ title: v.title, videoId: v.videoId, url: v.url, seconds: v.seconds }));

      if (v.seconds > 600) {
        return m.reply("Maksimal durasi 10 menit.");
      }
